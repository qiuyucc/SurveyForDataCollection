    ...
    public class AccountController : Controller
    {
       // private ApplicationDbContext db = new ApplicationDbContext();
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            //Session["LoggedUserName"] = model.UserName.ToString();
            //Session["LoggedUserPoint"] = model.point.ToString();
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
               // var role = model.RoleName.Equals("Memeber");
                var user = new ApplicationUser { UserName = model.UserName, Email = model.Email,point=0};
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
